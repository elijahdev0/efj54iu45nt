...
namespace FSPoster\App\Libraries\PHPImage;
...
class PHPImage
{
	...
	{
		return __DIR__ . '/../PHPImage/font/arial.ttf';
	}
	...
	public function __construct ( $mixed = null, $height = null )
	{
		...
		{
			$this->handleError( fsp__('You should install and enable GD, libjpeg, libpng, and freetype libraries.') );
		}
	 ...
	 * @return $this
	 */
	public function initialiseCanvas ( $width, $height, $resource = 'img', $color = [ 0, 0, 0, 127 ] )
	{
		$this->width  = $width;
		$this->height = $height;
		unset( $this->$resource );
		...
		imagefilledrectangle( $this->$resource, 0, 0, $this->width, $this->height, imagecolorallocatealpha( $this->$resource, $color[ 0 ], $color[ 1 ], $color[ 2 ], $color[ 3 ] ) );
	 ...
	 * @return $this
	 */
	public function batchResize ( $path, $dimensions = [] )
	{
		if ( is_array( $dimensions ) && count( $dimensions ) > 0 )
		{
			...
			foreach ( $dimensions as $args )
			{
				...
				call_user_func_array( [ $this, 'resize' ], $args );
	...
	public function line (
		$x1 = 0, $y1 = 0, $x2 = 100, $y2 = 100, $colour = [
		0,
		0,
		0,
	], $opacity = 1.0, $dashed = false
	) {
		if ( $dashed === true )
		{
	...
	public function rectangle (
		$x = 0, $y = 0, $width = 100, $height = 50, $colour = [
		0,
		0,
		0,
	 ...
	 * @see http://www.php.net/manual/en/function.imagefilledrectangle.php
	 */
	public function square ( $x = 0, $y = 0, $width = 100, $colour = [ 0, 0, 0 ], $opacity = 1.0, $outline = false )
	{
	 ...
	 * @see http://www.php.net/manual/en/function.imagefilledellipse.php
	 */
	public function circle ( $x = 0, $y = 0, $width = 100, $colour = [ 0, 0, 0 ], $opacity = 1.0, $outline = false )
	{
	 ...
	 */
	public function polygon ( $points = [], $colour = [ 0, 0, 0 ], $opacity = 1.0, $outline = false )
	{
		if ( count( $points ) > 0 )
		{
			if ( $outline === true )
	 ...
	 */
	public function arc (
		$x = 0, $y = 0, $width = 100, $height = 50, $start = 0, $end = 180, $colour = [
		0,
		0,
		0,
	], $opacity = 1.0, $outline = false
	 ...
	 */
	public function draw ( $file, $x = '50%', $y = '50%', $wr = null, $hr = null )
	{
		if ( $info = $this->getImageInfo( $file ) )
		{
			...
			if ( strpos( $x, '%' ) === false && ! is_numeric( $x ) && ! in_array( $x, [ 'left', 'center', 'right' ] ) )
			{
				$x = '50%';
			}
			if ( strpos( $y, '%' ) === false && ! is_numeric( $y ) && ! in_array( $y, [ 'top', 'center', 'bottom' ] ) )
			{
				$y = '50%';
			}
			...
			// Draw image
			imagecopyresampled( $this->img, $image, $x + ( $width - $wr ) / 2, $y + ( $height - $hr ) / 2, 0, 0, $wr, $hr, $width, $height );
			imagedestroy( $image );
			$this->afterUpdate();
	...
	public function text ( $text, $options = [] )
	{
		...
		foreach ( $lines as $index => $line )
		{
			...
			if ( is_int( $width ) || is_int( $height ) )
			{
				...
				if ( $debug )
				{
					$this->rectangle( $x + $offsetx, $lineY + $offsety - $fontHeight, $actualWidth, $lineHeight, [
						rand( 150, 255 ),
						rand( 150, 255 ),
						rand( 150, 255 ),
					], 0.5 );
	 ...
	 * @return integer
	 */
	protected function fitToBounds ( $fontSize, $angle, $fontFile, $text, $width, $height, $isRTL = false )
	{
		while ( $fontSize > 0 )
		{
			$wrapped      = $this->wrap( $text, $width, $fontSize, $angle, $fontFile, $isRTL );
	 ...
	 * @return $this
	 */
	public function textBox ( $text, $options = [], $isRTL = false )
	{
		$defaults = [
			'fontSize'        => $this->fontSize,
			'fontColor'       => $this->textColor,
		...
		if ( $height )
		{
			$fontSize = $this->fitTobounds( $fontSize, $angle, $fontFile, $text, $width, $height, $isRTL );
		}

		return $this->text( $this->wrap( $text, $width, $fontSize, $angle, $fontFile, $isRTL ), [
			'fontSize'        => $fontSize,
			'x'               => $x,
			'y'               => $y,
			'width'           => $width,
	 ...
	 * @return String
	 */
	protected function wrap ( $text, $width = 100, $fontSize = 12, $angle = 0, $fontFile = null, $isRTL = false )
	{
		if ( $fontFile === null )
		{
			$fontFile = $this->fontFile;
		...
		$ret = "";
		$arr = explode( ' ', $text );
        $arr = $isRTL ? array_reverse( $arr ) : $arr;
        $res = [];
	 ...
	 * @return $this
	 */
	public function setTextColor ( $colour = [ 255, 255, 255 ] )
	{
	 ...
	 * @return $this
	 */
	public function setStrokeColor ( $colour = [ 0, 0, 0 ] )
	{
	...
	public function setFont ( $fontFile )
	{
		if ( empty( $fontFile ) || ! file_exists( $fontFile ) )
			$fontFile = PHPImage::getDefaultFontFile();
