...
use FSPoster\App\SocialNetworks\Mastodon\Api\PostingData;
...
class PostingDataAdapter
{
	...
	public function getPostingData (): PostingData
	{
		...
		if ( !empty( $postingData->link ) )
			$cut = $cut - mb_strlen( $postingData->link ) - 1;

		$postingData->message = Helper::cutText( $postingData->message, $cut );
